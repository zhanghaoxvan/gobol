...
local function setup_highlight_autocmds(bufnr)
	vim.api.nvim_create_autocmd("CursorHold", {
		buffer = bufnr,
		callback = function()
			vim.lsp.buf_document_highlight()
		end,
	})
	vim.api.nvim_create_autocmd("CursorMoved", {
		buffer = bufnr,
		callback = function()
			vim.lsp.buf_clear_references()
		end,
	})
end
...
function M.start(bufnr)
	...
	if toml_path and opts.detect_grape then
		...
		if info then
			...
			vim.notify(
				string.format(
					"[gobol] Project: %s v%s (entry: %s, deps: %d)",
					info.name,
					info.version,
					info.entry,
					vim.tbl_count(info.dependencies)
				),
				vim.log.levels.INFO
			)
		elseif err then
...
local function setup_highlight_autocmds(bufnr)
	local aug = vim.api.nvim_create_augroup("GobolLspHighlight_" .. bufnr, { clear = true })
